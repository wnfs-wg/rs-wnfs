...
impl<'a> HashBits<'a> {
    ...
    pub fn next(&mut self, i: u32) -> Result<u32> {
        if i > 8 {
            return error(HamtError::InvalidHashBitLen);
        }
        if (self.consumed + i) as usize > self.b.len() * 8 {
            return error(HamtError::MaxDepth);
        }
        Ok(self.next_bits(i))
    }
...
mod tests {
    ...
    fn test_bitfield() {
        ...
        assert!(matches!(hb.next(9), Err(_))); //TODO(matheus23) figure out matching actual error HamtError::InvalidHashBitLen
        for _ in 0..28 {
            // Iterate through rest of key to test depth
            hb.next(8).unwrap();
        }
        assert!(matches!(hb.next(1), Err(_))); // TODO(matheus23) HamtError::MaxDepth
    }
}
