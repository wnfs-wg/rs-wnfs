use std::{io::{BufWriter, SeekFrom}, path::PathBuf, fs::File, collections::HashMap, sync::RwLock, borrow::Cow};
...
impl BlockStore for CarBlockStore {
    ...
    async fn get_block(&self, cid: &Cid) -> Result<Cow<Vec<u8>>> {
        ...
        // open the CAR file
        let mut car_file = File::open(inner.directory.join(format!("{}.car", location.car_number)))?;
    ...
    async fn put_block(&self, bytes: Vec<u8>, codec: IpldCodec) -> Result<Cid> {
        ...
        if inner.current_car.is_none() || inner.current_size + block_length as usize + block_length_bytes.len() > self.max_size {
        ...
        inner.current_car.as_mut().unwrap().write_all(&block_length_bytes)?;
...
mod tests {
    ...
    #[test]
    fn test_car_block_store() {
        let dir = tempdir().unwrap();
        let store = CarBlockStore::new(dir.path(), 1000).unwrap();
        let cid = store.put_block(vec![1,2,3], IpldCodec::Raw).unwrap();
